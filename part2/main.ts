...
// we have a goat now (same counter repo)
const deliveryGoat = new DeliveryObject(pizzaCounterMap, pairToUniqueString);

// iterate through movements
for(let k=0; k<movements.length; k+=2) {
    // move and deliver to that location
    deliveryPerson.move(movements[k]);
    // goat takes next move
    if (k+1 < movements.length) {
        deliveryGoat.move(movements[k+1]);
    }
}

// report time!
console.log('Part2 - Pizza delivery problem')
console.log('------------------------------')
